...
Шаг 2 — ВЫПОЛНЯЕТСЯ КОДОМ АВТОМАТИЧЕСКИ: как только клиент назвал породу —
         система отправит короткий комплимент породе, затем все доступные услуги от простого
         к дорогому с рекомендацией комплексного. Тебе НЕ НУЖНО и ЗАПРЕЩЕНО делать это самостоятельно.
...
def _breed_compliment(breed):
    name = re.sub(r'\s*\d+.*$', '', breed or '').strip()
    if not name:

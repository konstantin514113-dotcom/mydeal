...
def book():
    ...
    try:
        ...
        # Telegram уведомление о новой онлайн-записи (можно указать несколько chat_id через запятую в TELEGRAM_CHAT_ID)
        tg_token = os.environ.get("TELEGRAM_BOT_TOKEN")
        tg_chat_ids_raw = os.environ.get("TELEGRAM_CHAT_ID", "")
        tg_chat_ids = [c.strip() for c in tg_chat_ids_raw.split(",") if c.strip()]
        if tg_token and tg_chat_ids:
            ...
            for chat_id in tg_chat_ids:
                try:
                    ...
                    print(f"BOOK TELEGRAM ERROR (chat_id={chat_id}): {e}", flush=True)
    except Exception as e:
...
def api_clients_summary():
    """Сводные цифры по всей базе клиентов (перенесено со страницы /admin/clients на /stats)."""
    if request.args.get("stats_pass") != "210722":
    ...
    try:
        ...
        for b in bookings:
            ...
            entry = by_phone.setdefault(phone, {"pets": set(), "visits": 0, "total": 0.0})
            pet = b.get("petName", "")
            if pet:
                entry["pets"].add(pet)
            entry["visits"] += 1
        ...
        clients = list(by_phone.values())
        return jsonify({
            "clients": len(clients),
            "pets": sum(len(c["pets"]) for c in clients),
            "returning": sum(1 for c in clients if c["visits"] >= 2),
            "revenue": round(sum(c["total"] for c in clients), 0),
        })
    except Exception as e:
